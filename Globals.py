...
class Character:
    def __init__(self):
        ...
        self.guild_skills_ranks = [tkinter.StringVar() for i in range(
            6)]  # Let the profession object handle what the guild skills are. This hold the vars the for the Misc rows
    ...
    def load(self):
        ...
        char_file = tkinter.filedialog.askopenfile(initialdir="Characters", filetypes=[("Text files", "*.txt")],
                                                   mode='r', title="Load GS4 Character")
        ...
        while line:
            ...
            parts = line.split(
                ":")  # Every field in the save file line is divided by semi-colons. Splitting on the semi-colon will give us the individual parts of the line.
            ...
            # For statistics, update the appropriate statistic with the listed value. All statistics will be recalculated later
                ...
                self.build_skills_list.insert(location,
                                              skills_panel.Create_Build_List_Skill(skills_panel.ML_Frame.interior(),
                                                                                   parts[0], parts[1], location,
                                                                                   "%s / %s (%s)" % (
                                                                                   skill.ptp_cost, skill.mtp_cost,
                                                                                   skill.max_ranks), parts[3], parts[4],
                                                                                   parts[2]))
            ...
            # The order of the build is determined by the order the maneuvers are listed in the file
                ...
                self.build_combat_maneuvers_list.insert(location, man_panel.Create_Build_List_Maneuver(
                    man_panel.ML_Frame.interior(), parts[0],
                    "Combat", parts[1], location, parts[3], parts[4], parts[2], (
                    man.cost_by_rank[0], man.cost_by_rank[1], man.cost_by_rank[2], man.cost_by_rank[3],
                    man.cost_by_rank[4])))
            elif read_mode == "shield":
                man = self.shield_maneuvers_list[parts[0]]
                location = len(self.build_shield_maneuvers_list)
                self.build_shield_maneuvers_list.insert(location, man_panel.Create_Build_List_Maneuver(
                    man_panel.ML_Frame.interior(), parts[0],
                    "Shield", parts[1], location, parts[3], parts[4], parts[2], (
                    man.cost_by_rank[0], man.cost_by_rank[1], man.cost_by_rank[2], man.cost_by_rank[3],
                    man.cost_by_rank[4])))
            elif read_mode == "armor":
                man = self.armor_maneuvers_list[parts[0]]
                location = len(self.build_armor_maneuvers_list)
                self.build_armor_maneuvers_list.insert(location, man_panel.Create_Build_List_Maneuver(
                    man_panel.ML_Frame.interior(), parts[0],
                    "Armor", parts[1], location, parts[3], parts[4], parts[2], (
                    man.cost_by_rank[0], man.cost_by_rank[1], man.cost_by_rank[2], man.cost_by_rank[3],
                    man.cost_by_rank[4])))
            ...
            # Post cap lines are treated almost the same as the precap lines.
                ...
                self.postcap_build_skills_list.insert(location, postcap_panel.Create_Postcap_Build_List_Skill(
                    postcap_panel.ML_Frame.interior(), parts[0], parts[1], location,
                    "%s / %s (%s)" % (skill.ptp_cost, skill.mtp_cost, skill.max_ranks), parts[2]))
                self.postcap_build_skills_list[location].PcP_Edit_Button.config(
                    command=lambda v=location: postcap_panel.Add_Edit_Button_Onclick(v))
            elif read_mode == "combat postcap":
                man = self.combat_maneuvers_list[parts[0]]
                location = len(self.postcap_build_combat_maneuvers_list)
                self.postcap_build_combat_maneuvers_list.insert(location,
                                                                postcap_panel.Create_Postcap_Build_List_Maneuver(
                                                                    postcap_panel.ML_Frame.interior(), parts[0],
                                                                    "Combat", parts[1], location, parts[2], (
                                                                    man.cost_by_rank[0], man.cost_by_rank[1],
                                                                    man.cost_by_rank[2], man.cost_by_rank[3],
                                                                    man.cost_by_rank[4])))
            elif read_mode == "shield postcap":
                man = self.shield_maneuvers_list[parts[0]]
                location = len(self.postcap_build_shield_maneuvers_list)
                self.postcap_build_shield_maneuvers_list.insert(location,
                                                                postcap_panel.Create_Postcap_Build_List_Maneuver(
                                                                    postcap_panel.ML_Frame.interior(), parts[0],
                                                                    "Shield", parts[1], location, parts[2], (
                                                                    man.cost_by_rank[0], man.cost_by_rank[1],
                                                                    man.cost_by_rank[2], man.cost_by_rank[3],
                                                                    man.cost_by_rank[4])))
                ...
                self.postcap_build_armor_maneuvers_list.insert(location,
                                                               postcap_panel.Create_Postcap_Build_List_Maneuver(
                                                                   postcap_panel.ML_Frame.interior(), parts[0],
                                                                   "Armor", parts[1], location, parts[2], (
                                                                   man.cost_by_rank[0], man.cost_by_rank[1],
                                                                   man.cost_by_rank[2], man.cost_by_rank[3],
                                                                   man.cost_by_rank[4])))
        ...
        if skills_panel.menu_size > 1:
            for j in range(2, skills_panel.menu_size + 1):
                if j > 2:
                    skills_panel.edit_order_menu["menu"].insert_command("end", label=j - 1, command=lambda
                        v=j - 1: skills_panel.vars_dialog_order.set(v))
                skills_panel.add_order_menu["menu"].insert_command("end", label=j, command=lambda
                    v=j: skills_panel.vars_dialog_order.set(v))
        ...
        if man_panel.combat_menu_size > 1:
            for j in range(2, man_panel.combat_menu_size + 1):
                if j > 2:
                    man_panel.edit_combat_order_menu["menu"].insert_command("end", label=j - 1, command=lambda
                        v=j - 1: man_panel.vars_dialog_order.set(v))
                man_panel.add_combat_order_menu["menu"].insert_command("end", label=j, command=lambda
                    v=j: man_panel.vars_dialog_order.set(v))
        ...
        if man_panel.shield_menu_size > 1:
            for j in range(2, man_panel.shield_menu_size + 1):
                if j > 2:
                    man_panel.edit_shield_order_menu["menu"].insert_command("end", label=j - 1, command=lambda
                        v=j - 1: man_panel.vars_dialog_order.set(v))
                man_panel.add_shield_order_menu["menu"].insert_command("end", label=j, command=lambda
                    v=j: man_panel.vars_dialog_order.set(v))
        ...
        if man_panel.armor_menu_size > 1:
            for j in range(2, man_panel.armor_menu_size + 1):
                if j > 2:
                    man_panel.edit_armor_order_menu["menu"].insert_command("end", label=j - 1, command=lambda
                        v=j - 1: man_panel.vars_dialog_order.set(v))
                man_panel.add_armor_order_menu["menu"].insert_command("end", label=j, command=lambda
                    v=j: man_panel.vars_dialog_order.set(v))
        ...
        if postcap_panel.skills_menu_size > 1:
            for j in range(2, postcap_panel.skills_menu_size + 1):
                if j > 2:
                    postcap_panel.edit_skill_order_menu["menu"].insert_command("end", label=j - 1, command=lambda
                        v=j - 1: postcap_panel.vars_dialog_order.set(v))
                postcap_panel.add_skill_order_menu["menu"].insert_command("end", label=j, command=lambda
                    v=j: postcap_panel.vars_dialog_order.set(v))
        ...
        if postcap_panel.combat_menu_size > 1:
            for j in range(2, postcap_panel.combat_menu_size + 1):
                if j > 2:
                    postcap_panel.edit_combat_order_menu["menu"].insert_command("end", label=j - 1, command=lambda
                        v=j - 1: postcap_panel.vars_dialog_order.set(v))
                postcap_panel.add_combat_order_menu["menu"].insert_command("end", label=j, command=lambda
                    v=j: postcap_panel.vars_dialog_order.set(v))
        ...
        if postcap_panel.shield_menu_size > 1:
            for j in range(2, postcap_panel.shield_menu_size + 1):
                if j > 2:
                    postcap_panel.edit_shield_order_menu["menu"].insert_command("end", label=j - 1, command=lambda
                        v=j - 1: postcap_panel.vars_dialog_order.set(v))
                postcap_panel.add_shield_order_menu["menu"].insert_command("end", label=j, command=lambda
                    v=j: postcap_panel.vars_dialog_order.set(v))
        ...
        if postcap_panel.armor_menu_size > 1:
            for j in range(2, postcap_panel.armor_menu_size + 1):
                if j > 2:
                    postcap_panel.edit_armor_order_menu["menu"].insert_command("end", label=j - 1, command=lambda
                        v=j - 1: postcap_panel.vars_dialog_order.set(v))
                postcap_panel.add_armor_order_menu["menu"].insert_command("end", label=j, command=lambda
                    v=j: postcap_panel.vars_dialog_order.set(v))
    ...
    def save(self):
        ...
        char_file = tkinter.filedialog.asksaveasfile(initialdir="Characters", defaultextension=".txt", mode='w',
                                                     title="Save GS4 Character As ...")
    ...
    def Update_Statistics(self):
        ...
        for i in range(101):
            STR = float(0) if self.statistics_list["Strength"].values_by_level[i].get() == "" else float(
                self.statistics_list["Strength"].values_by_level[i].get())
            CON = float(0) if self.statistics_list["Constitution"].values_by_level[i].get() == "" else float(
                self.statistics_list["Constitution"].values_by_level[i].get())
            DEX = float(0) if self.statistics_list["Dexterity"].values_by_level[i].get() == "" else float(
                self.statistics_list["Dexterity"].values_by_level[i].get())
            AGI = float(0) if self.statistics_list["Agility"].values_by_level[i].get() == "" else float(
                self.statistics_list["Agility"].values_by_level[i].get())
            DIS = float(0) if self.statistics_list["Discipline"].values_by_level[i].get() == "" else float(
                self.statistics_list["Discipline"].values_by_level[i].get())
            AUR = float(0) if self.statistics_list["Aura"].values_by_level[i].get() == "" else float(
                self.statistics_list["Aura"].values_by_level[i].get())
            LOG = float(0) if self.statistics_list["Logic"].values_by_level[i].get() == "" else float(
                self.statistics_list["Logic"].values_by_level[i].get())
            INT = float(0) if self.statistics_list["Intuition"].values_by_level[i].get() == "" else float(
                self.statistics_list["Intuition"].values_by_level[i].get())
            WIS = float(0) if self.statistics_list["Wisdom"].values_by_level[i].get() == "" else float(
                self.statistics_list["Wisdom"].values_by_level[i].get())
            INF = float(0) if self.statistics_list["Influence"].values_by_level[i].get() == "" else float(
                self.statistics_list["Influence"].values_by_level[i].get())
            ...
            PTP_sum = (PTP_sum + STR * self.Is_Prime_Stat("Strength") + CON * self.Is_Prime_Stat(
                "Constitution") + DEX * self.Is_Prime_Stat("Dexterity") + AGI * self.Is_Prime_Stat("Agility")) / 20
            PTP_sum += 25

            MTP_sum = (AUR * self.Is_Prime_Stat("Aura") + DIS * self.Is_Prime_Stat("Discipline")) / 2
            MTP_sum = (MTP_sum + LOG * self.Is_Prime_Stat("Logic") + INT * self.Is_Prime_Stat(
                "Intuition") + WIS * self.Is_Prime_Stat("Wisdom") + INF * self.Is_Prime_Stat("Influence")) / 20
            MTP_sum += 25
    ...
    def Update_Resources(self, level):
        ...
        PF_ranks = 0 if not "Physical Fitness" in self.skills_list else \
        self.skills_list["Physical Fitness"].total_ranks_by_level[i].get()
        H_str = float(0) if self.statistics_list["Strength"].values_by_level[0].get() == "" else float(
            self.statistics_list["Strength"].values_by_level[0].get())
        H_con = float(0) if self.statistics_list["Constitution"].values_by_level[0].get() == "" else float(
            self.statistics_list["Constitution"].values_by_level[0].get())
        base_con = math.floor((H_str + H_con) / 10)
        con_bonus = float(0) if self.statistics_list["Constitution"].values_by_level[i].get() == "" else float(
            self.statistics_list["Constitution"].values_by_level[i].get())
        con_bonus = int(math.floor((con_bonus - 50) / 2 + self.racial_stat_bonus["Constitution"].get()))
        self.health_by_level[i].set(max(0, min(base_con + PF_ranks * (health_gain_rate + math.floor(con_bonus / 10)),
                                               self.race.max_health + con_bonus)))
        ...
        HP_ranks = 0 if not "Harness Power" in self.skills_list else \
        self.skills_list["Harness Power"].total_ranks_by_level[i].get()
        ...
        # Stamina calculation.
        PF_bonus = 0 if not "Physical Fitness" in self.skills_list else \
        self.skills_list["Physical Fitness"].bonus_by_level[i].get()
    ...
    def Update_Skills(self):
        ...
        for name in skill_names:
            if self.skills_list[name].active_skill:
                skill_panel.dialog_menu_skill_names['menu'].add_command(label=name, command=lambda
                    s=name: skill_panel.Skills_Menu_Onchange(s))
                postcap_panel.dialog_skill_names_menu['menu'].add_command(label=name, command=lambda
                    s=name: postcap_panel.Dialog_Menu_Onchange(s))
    ...
    def Update_Maneuvers(self):
        ...
        for name in combat_maneuver_names:
            ...
            if man.availability[prof]:
                man_panel.dialog_combat_names_menu['menu'].add_command(label=name, command=lambda
                    s=name: man_panel.Dialog_Menu_Onchange(s))
                postcap_panel.dialog_combat_names_menu['menu'].add_command(label=name, command=lambda
                    s=name: postcap_panel.Dialog_Menu_Onchange(s))
        for name in shield_maneuver_names:
            man = self.shield_maneuvers_list[name]
            man.Set_To_Default()
            if man.availability[prof]:
                man_panel.dialog_shield_names_menu['menu'].add_command(label=name, command=lambda
                    s=name: man_panel.Dialog_Menu_Onchange(s))
                postcap_panel.dialog_shield_names_menu['menu'].add_command(label=name, command=lambda
                    s=name: postcap_panel.Dialog_Menu_Onchange(s))
        for name in armor_maneuver_names:
            man = self.armor_maneuvers_list[name]
            man.Set_To_Default()
            if man.availability[prof]:
                man_panel.dialog_armor_names_menu['menu'].add_command(label=name, command=lambda
                    s=name: man_panel.Dialog_Menu_Onchange(s))
                postcap_panel.dialog_armor_names_menu['menu'].add_command(label=name, command=lambda
                    s=name: postcap_panel.Dialog_Menu_Onchange(s))
        ...
        if prof == "Warrior" or prof == "Rogue" or prof == "Paladin":
            ...
            man_panel.man_select_menu["menu"].insert_command("end", label="Combat", command=lambda
                s="Combat": man_panel.Maneuver_Style_Onchange(s))
            man_panel.man_select_menu["menu"].insert_command("end", label="Shield", command=lambda
                s="Shield": man_panel.Maneuver_Style_Onchange(s))
            man_panel.man_select_menu["menu"].insert_command("end", label="Armor", command=lambda
                s="Armor": man_panel.Maneuver_Style_Onchange(s))
            postcap_panel.man_select_menu["menu"].insert_command("end", label="Combat", command=lambda
                s="Combat": postcap_panel.PostCap_Style_Onchange(s))
            postcap_panel.man_select_menu["menu"].insert_command("end", label="Shield", command=lambda
                s="Shield": postcap_panel.PostCap_Style_Onchange(s))
            postcap_panel.man_select_menu["menu"].insert_command("end", label="Armor", command=lambda
                s="Armor": postcap_panel.PostCap_Style_Onchange(s))
        else:
            man_panel.man_select_menu["menu"].insert_command("end", label="Combat", command=lambda
                s="Combat": man_panel.Maneuver_Style_Onchange(s))
            postcap_panel.man_select_menu["menu"].insert_command("end", label="Combat", command=lambda
...
class Race:
    def __init__(self, arr):
        ...
        self.statistic_bonus = {"Strength": arr['strength_bonus'], "Constitution": arr['constitution_bonus'],
                                "Dexterity": arr['dexterity_bonus'], "Agility": arr['agility_bonus'],
                                "Discipline": arr['discipline_bonus'], "Aura": arr['aura_bonus'],
                                "Logic": arr['logic_bonus'], "Intuition": arr['intuition_bonus'],
                                "Wisdom": arr['wisdom_bonus'], "Influence": arr['influence_bonus']}
        self.statistic_adj = {"Strength": arr['strength_adj'], "Constitution": arr['constitution_adj'],
                              "Dexterity": arr['dexterity_adj'], "Agility": arr['agility_adj'],
                              "Discipline": arr['discipline_adj'], "Aura": arr['aura_adj'], "Logic": arr['logic_adj'],
                              "Intuition": arr['intuition_adj'], "Wisdom": arr['wisdom_adj'],
                              "Influence": arr['influence_adj']}
...
class Profession:
    def __init__(self, arr):
        ...
        self.guild_skills = [arr['guild_skill1'], arr['guild_skill2'], arr['guild_skill3'], arr['guild_skill4'],
                             arr['guild_skill5'], arr['guild_skill6']]
        self.statistic_growth = {"Strength": arr['strength_growth'], "Constitution": arr['constitution_growth'],
                                 "Dexterity": arr['dexterity_growth'], "Agility": arr['agility_growth'],
                                 "Discipline": arr['discipline_growth'], "Aura": arr['aura_growth'],
                                 "Logic": arr['logic_growth'], "Intuition": arr['intuition_growth'],
                                 "Wisdom": arr['wisdom_growth'], "Influence": arr['influence_growth']}
...
class Statistic:
    ...
    def Create_Statistic_Row_Frame(self, panel, stat):
        ...
        tkinter.Label(frame, width="10", bg="lightgray", textvar=self.parent.racial_stat_bonus[stat]).grid(row=0,
                                                                                                           column=1,
                                                                                                           padx="1",
                                                                                                           pady="1")
        tkinter.Label(frame, width="10", bg="lightgray", textvar=self.parent.stat_adj[stat]).grid(row=0, column=2,
                                                                                                  padx="1", pady="1")
        self.entrybox = tkinter.Entry(frame, width="6", justify="center", validate="key", validatecommand=mycmd,
                                      textvariable=var)
    ...
    def Create_Growth_Row_Frame(self, parent):
        ...
        for i in range(101):
            tkinter.Label(frame, width=6, bg=self.StP_display_bgs[i], textvariable=self.StP_display_values[i]).grid(
...
class Skill:
    ...
    def Calculate_TP_Regain(self, start, end):
        ...
        for i in range(start, end + 1):
            ...
            if self.total_ranks_by_level[i].get() == self.total_ranks_by_level[100].get() and (
                    self.ptp_regained_at_level[i].get() == 0 and self.mtp_regained_at_level[i].get() == 0):
                break
    ...
    def Train_Postcap_Ranks(self, ranks, subskill_ranks, exp):
        ...
        if exp in character.postcap_skill_training_by_interval.keys():
            if self.name in character.postcap_skill_training_by_interval[exp]:
                new_training = character.postcap_skill_training_by_interval[exp].replace(
                    "%s:%s" % (self.name, self.postcap_ranks_at_interval[exp] - ranks),
                    "%s:%s" % (self.name, self.postcap_ranks_at_interval[exp]))
                character.postcap_skill_training_by_interval.update({exp: new_training})
            else:
                character.postcap_skill_training_by_interval.update(
                    {exp: "%s|%s:%s" % (character.postcap_skill_training_by_interval[exp], self.name, ranks)})
        else:
...
class Maneuver:
    def __init__(self, schedule_parent, arr):
        ...
        self.prerequisites_displayed = re.sub("(CM:)|(SM:)|(GS:)|(Skill:)", lambda m: "%s " % m.group(0) or '',
                                              self.prerequisites)
    ...
    def Train_Postcap_Ranks(self, exp, ranks):
        ...
        if exp in training_by_interval.keys():
            if self.name in training_by_interval[exp]:
                new_training = training_by_interval[exp].replace(
                    "%s:%s" % (self.name, self.postcap_ranks_at_interval[exp] - ranks),
                    "%s:%s" % (self.name, self.postcap_ranks_at_interval[exp]))
...
class Information_Dialog:
    def __init__(self):
        ...
        self.myframe = Pmw.ScrolledFrame(self.dialogbox.interior(), usehullsize=1, hull_width=self.width,
                                         hull_height=self.height - 35)
        ...
        tkinter.Label(self.myframe_inner, anchor="w", font=self.error_font, wraplength=self.width - 10, justify="left",
                      textvariable=self.message).grid(row=0, column=0, sticky="w")
...
statistics = ["Strength", "Constitution", "Dexterity", "Agility", "Discipline", "Aura", "Logic", "Intuition", "Wisdom",
              "Influence"]
professions = ["Bard", "Cleric", "Empath", "Monk", "Paladin", "Ranger", "Rogue", "Sorcerer", "Warrior", "Wizard"]
races = ["Aelotoi", "Burghal Gnome", "Dark Elf", "Dwarf", "Elf", "Erithian", "Forest Gnome", "Giantman", "Half Elf",
         "Half Krolvin", "Halfling", "Human", "Sylvankind"]
